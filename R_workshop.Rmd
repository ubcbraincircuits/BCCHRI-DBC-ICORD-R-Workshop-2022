...
Everything in this white area is part of the markdown section of the Rmarkdown file. This means that it is just plain text that cannot be run in R. We can then create a coding section using three grave accents (this thing ` in the upper left corner) and {r}
```{r}
...
# Let's actually run something by hitting the green arrow at the top of the block
2+2

# You should be able to see the output below and in the console window. 
```

Hitting the green arrow at the top of the block will run everything in the block, but we can also run things line by line. We can run just one line of code by hitting command + return on a Mac or control + enter for Windows and Linux. 
...
quantile(x = vec2, na.rm = TRUE, type = 7)
```
